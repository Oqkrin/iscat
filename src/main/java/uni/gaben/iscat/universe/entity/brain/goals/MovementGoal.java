...
public interface MovementGoal {
    ...
    static MovementGoal evade(Target target) {
        // Hoisted vectors (Zero GC per frame)
        ...
        return (self, maxVelocity, universe, dt) -> {
            ...
            // Safe access: If no threats exist, stand still.
            if (targets == null || targets.isEmpty()) {
                return evadeVelocity.set(0, 0);
            }

            AbstractEntityModel entity = targets.get(0);

            // If the threat is despawning, stop running away from a ghost
            if (entity.shouldRemove()) {
                return evadeVelocity.set(0, 0);
            }
            ...
            // Set the flee velocity to full speed
